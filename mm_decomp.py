...
class SnappyGUI:
    def __init__(self, master):
        self.master = master
        self.master.title("MetaMask Data Decompressor v1.6")
        ...
        tk.Button(input_frame, text="Browse...", command=self.browse_input_file,
                  bg="#4CAF50", fg="white", activebackground="#2E7D32", activeforeground="white").pack(side=tk.RIGHT, padx=5, pady=5)
        ...
        tk.Button(output_frame, text="Save As...", command=self.browse_output_file,
                  bg="#4CAF50", fg="white", activebackground="#2E7D32", activeforeground="white").pack(side=tk.RIGHT, padx=5, pady=5)
        ...
        self.decrypt_button = tk.Button(button_frame, text="Decrypt MetaMask Data", command=self.decrypt_metamask_data, state=tk.DISABLED,
        ...
        # Bind Ctrl+D to launch vault_decryptor.py (supports both left and right Ctrl)
        self.master.bind('<Control-d>', lambda event: self.decrypt_metamask_data())

        self.master.protocol("WM_DELETE_WINDOW", self.on_closing)

    def decrypt_metamask_data(self):
        # Spustí vault_decryptor.py bez ohľadu na prítomnosť MetaMask údajov
        try:
            content = self.metamask_vault_text.get(1.0, tk.END).strip()
            # Ak sú údaje prítomné a nie sú chybové, prenes ich
            if content and "MetaMask vault data not found" not in content:
                encoded_content = base64.b64encode(content.encode('utf-8')).decode('utf-8')
                subprocess.Popen(['python', 'vault_decryptor.py', encoded_content])
            else:
                # Ak nie sú údaje, spusti vault_decryptor.py bez argumentov
                subprocess.Popen(['python', 'vault_decryptor.py'])
        except Exception as e:
            messagebox.showerror("Error", f"Failed to launch decryptor: {e}")
    ...
    def update_gui_from_queue(self):
        ...
        while not self.message_queue.empty():
            try:
                ...
                # Aktualizácia pokroku s popisomrape akcie
